...
You can use `Zarr <https://zarr.dev/>`_ to open NLR h5 resource files hosted on AWS S3 on your local computer. In our internal tests, this has comparable performance to reading data with an `HSDS <https://natlabrockies.github.io/rex/misc/examples.hsds.html>`_ local server. The benefit of this approach is that you don't need to run an HSDS server, the drawback is that you need to handle a large meta data file for every .h5 file you access on S3.
